...
public class HistoryDao extends AbstractDao<History, Long> {
    ...
    public static class Properties {
        ...
        public final static Property ReceiveId = new Property(2, Long.class, "receiveId", false, "RECEIVE_ID");
    }
    ...
    public History readEntity(Cursor cursor, int offset) {
        History entity = new History( //
            ...
            cursor.isNull(offset + 2) ? null : cursor.getLong(offset + 2) // receiveId
        );
        return entity;
    }
    ...
    public void readEntity(Cursor cursor, History entity, int offset) {
        ...
        entity.setReceiveId(cursor.isNull(offset + 2) ? null : cursor.getLong(offset + 2));
     }
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getSendDao().getAllColumns());
            builder.append(',');
            SqlUtils.appendColumns(builder, "T1", daoSession.getReceiveDao().getAllColumns());
            builder.append(" FROM HISTORY T");
            builder.append(" LEFT JOIN SEND T0 ON T.\"SEND_ID\"=T0.\"_id\"");
            builder.append(" LEFT JOIN RECEIVE T1 ON T.\"RECEIVE_ID\"=T1.\"_id\"");
            builder.append(' ');
    ...
    protected History loadCurrentDeep(Cursor cursor, boolean lock) {
        ...
        Send sendFile = loadCurrentOther(daoSession.getSendDao(), cursor, offset);
        entity.setSendFile(sendFile);
        offset += daoSession.getSendDao().getAllColumns().length;

        Receive receiveFile = loadCurrentOther(daoSession.getReceiveDao(), cursor, offset);
        entity.setReceiveFile(receiveFile);
